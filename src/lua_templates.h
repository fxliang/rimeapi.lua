...
struct LUAWRAPPER_LOCAL LuaTypeInfo {
  ...
  const char *name() const {
    ...
    const char* name_ = ti->name();
    int status = 0;
    char* demangled = abi::__cxa_demangle(name_, nullptr, nullptr, &status);
    if (status == 0 && demangled) {
      thread_demangle_cache[hash] = demangled;
      free(demangled);
      return thread_demangle_cache[hash].c_str();
    } else {
      thread_demangle_cache[hash] = name_;
      return name_;
    }
#else
    return ti->name();
